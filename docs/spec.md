...
- `ret`: 戻り値型 `"i32"` / `"bool"` / `"str"` / `"f64"` / `"(i32, i32)"` / `"bytes"` / 省略で `()`
...
match result {
    m::Value::Array(items) => {
        for item in items {
            if let m::Value::Map(entry) = item {
                if let Some(m::Value::Bool(true)) = entry.get("is_entity") {
                    // エンティティ処理
                }
            }
        }
    },
    _ => eprintln!("unexpected return type")
}
```
