...
// Phase 1: 結果サイズを確認（offset-by-1 エンコーディング）
host_poll_result(request_id: i64) → i64
  → 0=pending, payload.length+1=ready (>=1), <0=error
  0バイト結果でも 1 が返る（pending の 0 と区別するため）。

// Phase 2: Rust が動的に確保したバッファに結果をコピー
host_fetch_result(request_id: i64, result_ptr: i32, result_buf_size: i32) → i32
  ...
  result_buf_size < 実際のサイズ の場合は ERR_BUFFER_TOO_SMALL を返す。
```
...
host_do_action(
  ...
  結果ポーリングは共通の host_poll_result / host_fetch_result を使用する。
```
...
| -2 | `BufferTooSmall` | 結果バッファが不足（`host_fetch_result` / `_imm` 系関数のみ） |
