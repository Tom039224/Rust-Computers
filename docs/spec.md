...
│   └── msgpack.rs      # MessagePack シリアライズヘルパー
├── peripherals/
...
- `ret`: 戻り値型 `"i32"` / `"bool"` / `"str"` / `"(i32, i32)"` / 省略で `()`
...
| W-6 | Chicory Runtime Compiler (JIT) 統合 | ⏳ **調査中** | Forge ClassLoader 競合のため保留 → 0.2.0 課題 |
...
- **シリアライゼーション**: MessagePack（Rust 側: 手書き, Java 側: msgpack-core 0.9.8）
