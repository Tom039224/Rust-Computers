...
**モジュール:** `create`  
**ペリフェラルタイプ:** `create:repackager`

Create Repackager ペリフェラル。インベントリの内容からパッケージを作成し、アドレス管理、パッケージの再梱包・送受信イベントの監視を行います。
...
パッケージ再梱包イベントを 1tick 待機して取得します（ノンブロッキング）。戻り値は `(CRPackage, count)` のタプル。
...
パッケージ再梱包イベントを受信するまで待機します（ブロッキング非同期ループ）。
