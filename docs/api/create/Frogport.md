...
**Module:** `create`  
**Peripheral Type:** `create:frogport`

Create Frogport peripheral. Manages package sending/receiving with address and inventory access. All methods are `mainThread=true`, so no imm variants.
...
Try to pull a package received event (non-blocking, returns `None` if no event).
