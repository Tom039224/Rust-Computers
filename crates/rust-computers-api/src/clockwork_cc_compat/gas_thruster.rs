...
impl GasThruster {
    pub async fn async_get_thrust(&mut self) -> Result<f64, PeripheralError> {
        self.book_next_get_thrust();
        crate::wait_for_next_tick().await;
        self.read_last_get_thrust()
    }

    pub async fn async_get_flow_rate(&mut self) -> Result<f64, PeripheralError> {
        self.book_next_get_flow_rate();
        crate::wait_for_next_tick().await;
        self.read_last_get_flow_rate()
    }

    pub async fn async_get_gas_mass_flow(&mut self) -> Result<alloc::collections::BTreeMap<alloc::string::String, f64>, PeripheralError> {
        self.book_next_get_gas_mass_flow();
        crate::wait_for_next_tick().await;
        self.read_last_get_gas_mass_flow()
    }
